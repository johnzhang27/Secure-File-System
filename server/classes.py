...
class Base(sqlorm.DeclartiveBase):
    pass
...
class User(Base):
    ...
    password: sqlorm.Mapped[str] = sqlorm.mapped_column()
    group_id: sqlorm.Mapped[str] = sqlorm.relationship(back_populates="users")
    owned_files: sqlorm.Mapped[typing.List["File"]] = sqlorm.relationship(back_populates="owner")
    permitted_files: sqlorm.Mapped[typing.List["File"]] = sqlorm.relationship(back_populates="permitted_users")

class Group(Base):
    __tablename__ = "GROUP_DATA"
    group_id: sqlorm.Mapped[str] = sqlorm.mapped_column(primary_key=True)
    users: sqlorm.Mapped[typing.List["User"]] = sqlorm.relationship(secondary=association_table,
                                                                    back_populates="group_id")

class File(Base):
    __tablename__ = "FILE_DATA"
    file_id: sqlorm.Mapped[str] = sqlorm.mapped_column(primary_key=True)
    parent: sqlorm.Mapped[str] = sqlorm.mapped_column()
    file_name: sqlorm.Mapped[str] = sqlorm.mapped_column()
